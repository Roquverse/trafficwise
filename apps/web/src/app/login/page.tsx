...
import { AtSign, Mail, Lock, EyeOff, Eye, LayoutGrid } from "lucide-react";
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-teal-950 flex-col justify-center items-center p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-md w-full bg-slate-900/40 backdrop-blur-xl border border-slate-700/50 p-10 rounded-[2rem] shadow-2xl">
          ...
          <h2 className="text-3xl font-semibold text-slate-100 mb-8 leading-snug">
            Predictive Resilience for Urban Mobility.
          </h2>

          <div className="flex items-start space-x-4">
            <div className="w-8 h-1 bg-primary mt-2.5 shrink-0 rounded-full" />
            <p className="text-slate-300 text-lg leading-relaxed">
              Intelligent routing powered by real-time network analytics.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="bg-slate-900/80 p-1.5 rounded-xl flex space-x-1 border border-slate-800">
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-colors ${role === "commuter"
            ...
            <button
              onClick={() => setRole("ops")}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-colors ${role === "ops"
                  ? "bg-slate-800 text-primary shadow-sm"
                  : "text-slate-400 hover:text-slate-300 hover:bg-slate-800/50"
                }`}
            >
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <Link href="#" className="text-xs font-semibold text-primary hover:text-amber-400 transition-colors">
                  Forgot?
                </Link>
              </div>
              <div className="relative">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full bg-primary hover:bg-amber-400 text-slate-950 font-bold py-3.5 rounded-lg transition-colors mt-2 shadow-[0_0_15px_rgba(245,158,11,0.2)] disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <p className="text-center text-slate-400 mt-8 text-sm">
            ...
            <Link href="/signup" className="font-semibold text-primary hover:text-amber-400 transition-colors">
              Sign up
            </Link>
          </p>
        </div>
